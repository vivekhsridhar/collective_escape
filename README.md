...
- `agent.py`: define an ordinary `Agent` class with an `__init__` constructor. Store each agent's behavioral state in `state`, its remaining escape time in `escape_timer`, and its remaining refractory time in `refractory_timer`.
- `stimulus.py`: define a `square_wave` function that returns a stimulus of specified amplitude for a fixed duration, and zero amplitude outside this timeframe.
